                            ...
                            {lectures.map((lecture, idx) => (
                                <tr key={lecture.lectureID || lecture.LectureID || idx} style={{ background: idx % 2 === 0 ? '#fffbe9' : '#fff', color: '#2C1810' }}>
                                    <td style={{ padding: '12px 14px', border: 'none' }}>
                                        <a href={`http://localhost/local/scholarwatch/api/serveLecture.php?file=${lecture.directoryPath.split('/').pop()}`} target="_blank" rel="noopener noreferrer" style={{ color: '#2C1810', fontWeight: 700, textDecoration: 'none', fontSize: '1.08rem' }}
